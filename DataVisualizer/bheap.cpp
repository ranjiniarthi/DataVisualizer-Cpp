    ...
    if (num_of_entries==1) {
       ...
       for (int pos=1; pos<num_of_entries; pos++) {
            ...
            if (new_height>heap_height) {
                Ui::EllipseElement *parent = hmap[pos/2];
        ...
        if (!(num_of_entries%2)) { // Insert the Left Child to the left of its parent
            Ui::EllipseElement *parent = hmap[num_of_entries/2];
            if (parent != NULL) {
                node->pos = new QPointF;
...
void Bheap::cleanupElem(Ui::EllipseElement *node)
{
...
{
    heap[idx-1] = heap[num_of_entries-1];
    ...
    cleanupElem(hmap[num_of_entries+1]);
